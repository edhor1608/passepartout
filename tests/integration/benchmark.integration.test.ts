...
const outDir = mkdtempSync(join(tmpdir(), "passepartout-benchmark-"));
...
describe("benchmark cli integration", () => {
  test("returns benchmark contract with score and grade", async () => {
    ...
    expect((confidence.value as number) <= 0.75).toBe(true);
    expect(typeof confidence.label).toBe("string");
    expect(["low", "medium"]).toContain(confidence.label as string);
  });
});
